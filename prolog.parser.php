 ...
 * Goal ::= "!" | "fail" | Predicate
 * Terms ::= Term ["," Term]*
 * Term ::= WORD ["(" Terms ")"]? | VARIABLE | DOT "(" Term "," Term ")" | "[]"
 * Query ::= "?-" Goals "."
 */
 ...
 *        | PS_DOT PS_LPAREN Term PS_COMMA Term PS_RPAREN | PS_EMPTYLIST
...
// end of tryNextToken
    // Code ::= Program Query
    function parse()
    { // parse and compile: returns WimCodeString
...
// structure(term1, term2, ..., termN)
            ...
            if (!$this->tryNextToken())
                return; // error occured!
        } elseif ($this->scanner->token == PS_DOT) {
            // . is a name of a tuple-structure (array-structure)
            $array = $this->scanner->tokenValue;
            ...
            if ($this->scanner->token == PS_LPAREN) {
                ...
                $twoterms = new PrologTerms($this);
                $twoterms->addTerm($this->gTerm());

                if ($this->scanner->token == PS_COMMA) {
                    if (!$this->tryNextToken())
                        return; // error occured!

                    $twoterms->addTerm($this->gTerm());

                    $result->setStructure($array, $twoterms);

                    if ($this->scanner->token == PS_RPAREN) {
                        if (!$this->tryNextToken())
                            return; // error occured!
                    } else {
                        $this->addError("'.'-structure needs exactly two terms.");
                    }
                } else {
                    $this->addError("'.'-structure needs exactly two terms.");
                }
            } else {
                $this->addError("'.' needs terms.");
            }
        } elseif ($this->scanner->token == PS_EMPTYLIST) {
            // [] is an atom
            $result->setAtom($this->scanner->tokenValue);
            if (!$this->tryNextToken())
                return; // error occured!
        } else {
